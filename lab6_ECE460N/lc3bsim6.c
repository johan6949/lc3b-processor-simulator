...
void dcache_access(int dcache_addr, int *read_word, int write_word, int *dcache_r, 
  ...
  if (!random) {
    *dcache_r = 0; //this is for it to stall -> not ready every 9th cycle
    *read_word = 0xfeed ;
  }
  else {
    *dcache_r = 1;
...
int compute_mem_pcmux() {

  if (PS.MEM_V && Get_BR_OP(PS.MEM_CS)) {

    int BEN = (bitsToDec(PS.MEM_CC, 1, 1, UNSIGNED) && bitsToDec(PS.MEM_IR, 10, 10, UNSIGNED)) || 
              (bitsToDec(PS.MEM_CC, 2, 2, UNSIGNED) && bitsToDec(PS.MEM_IR, 11, 11, UNSIGNED)) ||
...
int trap_pc, target_pc, v_mem_ld_reg, v_mem_ld_cc, mem_pcmux;
/************************* MEM_stage() *************************/
void MEM_stage() {
  ...
  if (Get_DCACHE_EN(PS.MEM_CS) & PS.MEM_V){
    ...
    int in = Get_DATA_SIZE(PS.MEM_CS) ? PS.MEM_ALU_RESULT :
            (bitsToDec(PS.MEM_ALU_RESULT, 7, 0, UNSIGNED) << 8) + bitsToDec(PS.MEM_ALU_RESULT, 7, 0, UNSIGNED);

    dcache_access(PS.MEM_ADDRESS, &out, in, &start, WE0, WE1);
  }
  if (!Get_DATA_SIZE(PS.MEM_CS)) {
    out = Low16bits((PS.MEM_ADDRESS & 1) ? bitsToDec(out, 15, 8, SIGNED) : bitsToDec(out, 7, 0, SIGNED));
  }
  mem_stall = (PS.MEM_V & Get_DCACHE_EN(PS.MEM_CS)) & !start;
  trap_pc = out;
  target_pc = PS.MEM_ADDRESS;

  mem_pcmux = compute_mem_pcmux();
...
int compute_adder(int addr1_mux) {

  int addr2mux = Get_ADDR2MUX(PS.AGEX_CS);
  int lshf1 = Get_LSHF1(PS.AGEX_CS);
  int offset;
  ...
  if (addr2mux == 0) {
    ...
    if (lshf1)
      offset = bitsToDec(PS.AGEX_IR, 5, 0, SIGNED) << 1;
    else
      offset = bitsToDec(PS.AGEX_IR, 4, 0, SIGNED);
  }
  else if (addr2mux == 2) {
    if (lshf1)
      offset = bitsToDec(PS.AGEX_IR, 8, 0, SIGNED) << 1;
    else
      offset = bitsToDec(PS.AGEX_IR, 4, 0, SIGNED);
  }
  else { // addr2mux == 3
    if (lshf1)
      offset = bitsToDec(PS.AGEX_IR, 10, 0, SIGNED) << 1;
    else
      offset = bitsToDec(PS.AGEX_IR, 4, 0, SIGNED);
  }

  return Low16bits(addr1_mux + offset);
}

int compute_shifter(int sr1, int ir) {

  int shift_amount = bitsToDec(ir, 3, 0, UNSIGNED);
  int mode_bit4 = bitsToDec(ir, 4, 4, UNSIGNED);
  int mode_bit5 = bitsToDec(ir, 5, 5, UNSIGNED);

  int result;

  if (!mode_bit4) {
    result = sr1 << shift_amount;
  }

  else if (mode_bit5) {
    if (bitsToDec(sr1, 15, 15, UNSIGNED)) {
      int temp = sr1;
      for (int i = shift_amount; i > 0; i--) {
        temp = (temp >> 1) | 0x8000;  // preserve sign bit
      }
      result = temp;
    } else {
      result = sr1 >> shift_amount;
...
int compute_alu(int sr1, int sr2_mux) {
...
void AGEX_stage() {
  ...
  adder = compute_adder(addr1_mux);

  int address_result = Get_ADDRESSMUX(PS.AGEX_CS) ? adder : bitsToDec(PS.AGEX_IR, 7, 0, UNSIGNED) << 1;
  int shifter;
  
  shifter = compute_shifter(PS.AGEX_SR1, PS.AGEX_IR);

  int sr2_mux = Get_SR2MUX(PS.AGEX_CS) ? bitsToDec(PS.AGEX_IR, 4, 0, SIGNED) : PS.AGEX_SR2;
  int alu;

  alu = compute_alu(PS.AGEX_SR1, sr2_mux);


  int alu_result = Get_ALU_RESULTMUX(PS.AGEX_CS) ? alu : shifter;
  ...
  if (LD_MEM) {
    ...
    NEW_PS.MEM_ADDRESS = address_result;
...
int compute_dep_stall(int control_store_address, int sr1, int sr2) {
...
void DE_stage() {
  ...
  int CONTROL_STORE_ADDRESS = (bitsToDec(PS.DE_IR, 15, 11, UNSIGNED) << 1) + bitsToDec(PS.DE_IR, 5, 5, UNSIGNED);  //row
  ...
  /* your code for DE stage goes here */
    ...
    int DRM = Get_DRMUX(CONTROL_STORE[CONTROL_STORE_ADDRESS]) ? 7 : bitsToDec(PS.DE_IR, 11, 9, UNSIGNED);
  
    dep_stall = compute_dep_stall(CONTROL_STORE_ADDRESS, SR1, SR2);
...
void FETCH_stage() {
  ...
  int next = PC + 2;
  int returnAddress;
  icache_access(PC, &returnAddress, &icache_r); //instruction fetch
  ...
  if (!mem_stall && !dep_stall){ //load DE latches
    NEW_PS.DE_NPC = next;
    NEW_PS.DE_IR = returnAddress;
    NEW_PS.DE_V = icache_r & (!v_mem_br_stall) & (!v_agex_br_stall) & (!v_de_br_stall); //no stall
  }
}  
