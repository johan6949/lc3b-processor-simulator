...
int eval_sp_bus() {
    switch (GetSPMux(CMI)) {
        case 0:
            return CURRENT_LATCHES.Saved_USP;

        case 1:
            return Low16bits(CURRENT_LATCHES.REGS[SR1] + 2);

        case 2:
            return Low16bits(CURRENT_LATCHES.REGS[SR1] - 2);

        case 3:
            return CURRENT_LATCHES.Saved_SSP;
...
int eval_alu() {
    int sr1_val = CURRENT_LATCHES.REGS[SR1];
    int op2;
    if (bitsToDec(CURRENT_LATCHES.IR, 5, 5, UNSIGNED)) {
        op2 = bitsToDec(CURRENT_LATCHES.IR, 4, 0, SIGNED); // imm5
    } else {
        op2 = CURRENT_LATCHES.REGS[SR2]; // SR2
    }

    switch (GetALUK(CMI)) {
        case 0: // ADD
            return Low16bits(sr1_val + op2);
...
void latch_datapath_values() {
    ...
    if (GetLD_BEN(CMI)) {
        int n_enable = bitsToDec(CURRENT_LATCHES.IR, 11, 11, UNSIGNED);
        int z_enable = bitsToDec(CURRENT_LATCHES.IR, 10, 10, UNSIGNED);
        int p_enable = bitsToDec(CURRENT_LATCHES.IR, 9, 9, UNSIGNED);

        NEXT_LATCHES.BEN =
            (CURRENT_LATCHES.N & n_enable) ||
            (CURRENT_LATCHES.Z & z_enable) ||
            (CURRENT_LATCHES.P & p_enable);
    }
    if (GetLD_IR(CMI)) {
        NEXT_LATCHES.IR = BUS;
    }
